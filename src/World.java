...
public class World extends JPanel {
    ...
    private final AtomicInteger generation; //keep track of what integer we're on
    ...
    private World(){
        ...
        //creates timer which wakes up every 5 milliseconds
    ...
    private void drawBestChromosome(Graphics2D g) {
        final java.util.List<TSP_Gene> chromosome = this.population.getAlpha().getChromosome();
        g.setColor(Color.WHITE);
        for (int i = 0; i < chromosome.size() - 1; i++){
            TSP_Gene gene = chromosome.get(i);
            TSP_Gene neighbor = chromosome.get(i+1);
            g.drawLine(gene.getX(), gene.getY(), neighbor.getX(), neighbor.getY());
        }
        g.setColor(Color.red);
        for (final TSP_Gene gene : chromosome){
